...
`verify-reference`와 `evaluate-export`는 Research-owned batch-1 Float contract를 통과해 exit code 0을 반환한다. `evaluate-int8`은 machine-readable evidence를 쓴 뒤 exact decision parity PASS와 continuous numerical contract FAIL을 반영해 exit code 2를 반환한다. Generic Vela 수치는 board timing이 아니다.
